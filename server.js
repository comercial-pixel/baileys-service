// server.js — Baileys API Corrigido e Estável
    ...
    sock.ev.on("connection.update", async (update) => {
      ...
      if (connection === "close") {
        ...
        if (code === DisconnectReason.loggedOut || code === 401) {
          log.warn({ sessionId, code }, "Session logged out — recreating");
          sessions.delete(sessionId);
          try {
            fs.rmSync(authDir, { recursive: true, force: true });
          } catch {}
          setTimeout(() => getOrCreateSession(sessionId), 1000);
          return;
        }

        log.warn({ sessionId, code }, "Connection closed — retrying");
        sessions.delete(sessionId);
        setTimeout(() => getOrCreateSession(sessionId), 2000);
      }
    });
...
// ------------------ HEALTH / INFO -------------------
app.get("/", (_, res) => res.json({ ok: true, service: "baileys-api" }));
...
app.post("/sessions/:id/send", requireAuth, async (req, res) => {
  ...
  try {
    const wa = await meta.sock.onWhatsApp(digits).catch(() => []);
    if (!wa || wa.length === 0 || wa[0]?.exists === false)
...
// ---------------------- START -----------------------
app.listen(PORT, () =>
  log.info(`✅ Baileys API escutando na porta ${PORT}`)
);
